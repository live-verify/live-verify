...
- **Rotating-salt gating:** the card has an active display (e-Ink or screen) showing a server-issued salt. The server rotates the salt — after each successful verification (burn-on-verify) or on TTL expiry of an unscanned salt — and the display re-renders to match, so the screen is always in lockstep with the salt the server will honour and only the currently-displayed code resolves to a photo. The subject doesn't need to tap an app — but the display's microcontroller still needs to learn the current salt, either via its own cellular/NFC radio or by Bluetooth-syncing from the subject's phone. The phone interaction moves from "press allow" to "be nearby with Bluetooth on", which is less friction but not zero dependency. See [E-Ink ID Cards](../e-ink-id-cards.md) for the full model.
