...
**Why would you bother verifying?** Because if the firm's PI cover has lapsed and the work goes wrong, the client has no insured party to claim against. In conveyancing, a negligent title search by an uninsured solicitor can leave the buyer with a property that has undisclosed charges or defective title — and no route to compensation. In architecture, a structural defect attributable to the architect's error may not emerge for years; if the firm had no cover when the work was done, the client bears the loss. The SRA intervenes in solicitor firms that practise without PI cover, but intervention happens after the damage — often after clients have already paid deposits, exchanged contracts, or relied on negligent advice. Checking before instructing the firm is the only point where the client can actually act on the information.
