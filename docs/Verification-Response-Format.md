...
Every verification response may include a `tx` object — a receipt of the verification event that the verifier's device retains. This provides evidence that a verification occurred, when, against which issuer, and what the result was.

```json
{
  "status": "verified",
  "tx": {
    "id": "VTX-2026-03-15-14h-8k3m9x",
    "timestamp": "2026-03-15T14:31:07Z",
    "hashVerified": "a7f3b2c9d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1",
    "issuerDomain": "wills.registry.gov.uk"
  }
}
```
...
| Field | Type | Description |
|-------|------|-------------|
| `tx.id` | string | Unique transaction identifier generated by the issuer. Format is issuer-defined but should be human-referenceable (not a raw UUID). |
| `tx.timestamp` | string | ISO 8601 timestamp of the verification event as recorded by the issuer's server. |
| `tx.hashVerified` | string | The hash that was looked up. Confirms which document this verification event relates to. |
| `tx.issuerDomain` | string | The domain that performed the verification. Redundant with the URL the verifier called, but included for self-contained record-keeping. |

**What the transaction record is:** Evidence that a verification event occurred — a receipt. The verifier's device stores it. The issuer's server log is the other half. Together they form a dual-record: the verifier can prove they checked, and the issuer can confirm the check happened.

**What the transaction record is not:** It is not evidence of what the document said (the verifier already has the document), nor proof of the verifier's identity (the issuer doesn't know who made the GET request unless additional authentication is used).

**Where transaction records are referenced later:**

The `tx.id` can be cited in due-process proceedings — not limited to courts of law:

| Context | How the transaction record is used |
|---------|-----------------------------------|
| **Professional conduct hearings** (SRA, GMC, NMC) | "I verified the practitioner's credentials before the procedure — transaction VTX-2026-03-15-14h-8k3m9x" |
| **Insurance claims** | "I verified the policy was active at the time of the incident" |
| **Employment tribunals** | "I verified the reference letter before making the hiring decision" |
| **Regulatory audits** | "We verified all supplier certificates on this date — here are the transaction IDs" |
| **Internal investigations** | "The bank teller verified the executor's Letters Testamentary before releasing funds" |
| **Ombudsman complaints** | "I verified the solicitor's letter was genuine before complying with its demands" |
| **Probate disputes** | "The witness verified the testator's identity at the signing ceremony" |
| **Compliance evidence** | "Our system verified every employee's right-to-work document — batch transaction log attached" |

**Retention:** The verifier's app should retain `tx` records for a reasonable period (aligned with the document type's typical retention — see individual use cases). The issuer retains the corresponding server-side log. Either party can independently produce their half of the record.

**Relationship to verification_id:** The existing `verification_id` field (Pattern 2 in Post-Verification Actions) serves a similar purpose for high-stakes interactions like law enforcement encounters, where the ID also links to a complaint URL. The `tx` object is more general — it applies to every verification event, not just those with post-verification actions. Where both are present, `verification_id` is the human-facing reference for the specific interaction, while `tx.id` is the technical receipt for the verification event itself.

**Optional, not required:** Issuers serving static files (e.g., GitHub Pages with `{"status": "verified"}`) cannot generate transaction records — they have no server-side logic. The `tx` field is optional. Its absence means the issuer doesn't support transaction tracking, not that the verification failed. Verifiers should not treat a missing `tx` as suspicious.
